...
void NetworkWidget::Update()
{
  ...
  for (u32 ssl_id = 0; ssl_id < IOS::HLE::NET_SSL_MAXINSTANCES; ssl_id++)
  {
    ...
    {
      host_fd =
          reinterpret_cast<mbedtls_net_context*>(IOS::HLE::Device::NetSSL::_SSL[ssl_id].ctx.p_bio)
              ->fd;
    }
...
QTableWidgetItem* NetworkWidget::GetSocketDomain(s32 host_fd) const
{
  if (host_fd < 0)
  ...
  if (ret != 0)
    return new QTableWidgetItem(tr("Unknown"));
...
QTableWidgetItem* NetworkWidget::GetSocketType(s32 host_fd) const
{
  if (host_fd < 0)
    return new QTableWidgetItem();
...
QTableWidgetItem* NetworkWidget::GetSocketState(s32 host_fd) const
{
  if (host_fd < 0)
    return new QTableWidgetItem();
  ...
  if (getpeername(host_fd, reinterpret_cast<struct sockaddr*>(&peer_addr), &peer_addr_len) == 0)
    return new QTableWidgetItem(tr("Connected"));
  ...
  if (ret == 0 && so_accept > 0)
    return new QTableWidgetItem(tr("Listening"));
  return new QTableWidgetItem(tr("Unbound"));
}

QTableWidgetItem* NetworkWidget::GetSocketName(s32 host_fd) const
{
  if (host_fd < 0)
    return new QTableWidgetItem();
  ...
  if (getsockname(host_fd, reinterpret_cast<struct sockaddr*>(&sock_addr), &sock_addr_len) != 0)
    return new QTableWidgetItem(tr("Unknown"));
