...
void ThreadWidget::CreateWidgets()
{
  ...
  Update();
  const Common::Debug::PartialContext empty_context;
  UpdateThreadContext(&empty_context);
}
...
QGroupBox* ThreadWidget::CreateThreadGroup()
{
  ...
  QStringList header;
  header << tr("Address") << tr("State") << tr("Detached") << tr("Suspended")
...
QGroupBox* ThreadWidget::CreateThreadCallstackGroup()
{
  ...
  QStringList header;
  header << tr("Address") << tr("Back Chain") << tr("LR Save") << tr("Description");
...
void ThreadWidget::Update()
{
  ...
  if (state == Core::State::Stopping)
  {
    ...
    const Common::Debug::PartialContext empty_context;
    UpdateThreadContext(&empty_context);
    UpdateThreadCallstack(&empty_context);
  }
  if (state != Core::State::Paused)
    return;

  const auto get_hex = [](u32 value) {
    return QStringLiteral("%1").arg(value, 8, 16, QLatin1Char('0'));
  };
  const auto get_f64 = [](double value) {
    return QStringLiteral("%1").arg(Common::BitCast<u64>(value), 16, 16, QLatin1Char('0'));
  };
  const auto get_hex_from = [&](u32 addr) {
    addr = PowerPC::HostIsRAMAddress(addr) ? PowerPC::HostRead_U32(addr) : 0;
    return get_hex(addr);
  };
  const auto get_string_from = [](u32 addr) {
    if (!PowerPC::HostIsRAMAddress(addr))
      return QStringLiteral("\"\"");
    return QStringLiteral("\"%1\"").arg(QString::fromStdString(PowerPC::HostGetString(addr)));
  };
  ...
  m_current_context->setText(get_hex_from(0x800000D4));
  m_current_thread->setText(get_hex_from(0x800000E4));
  m_default_thread->setText(get_hex_from(0x800000D8));

  m_queue_head->setText(get_hex_from(0x800000DC));
  m_queue_tail->setText(get_hex_from(0x800000E0));
  ...
  for (const auto& thread : m_threads)
  {
    ...
    m_thread_table->setItem(i, 0, new QTableWidgetItem(get_hex(thread->GetAddress())));
...
{
  const auto get_hex = [](const std::optional<u32>& value) {
    if (!value)
      return QString{};
    return QStringLiteral("%1").arg(*value, 8, 16, QLatin1Char('0'));
  };
  const auto get_hex_idx = [](const auto& table, std::size_t index) {
    if (!table || index >= table->size())
      return QString{};
    return QStringLiteral("%1").arg(table->at(index), 8, 16, QLatin1Char('0'));
  };
  const auto get_f64_idx = [](const auto& table, std::size_t index) {
    if (!table || index >= table->size())
      return QString{};
  ...
  for (int i = 0; i < 32; i++)
  {
    ...
    m_context_table->setItem(i, 1, new QTableWidgetItem(get_hex_idx(context->gpr, i)));
    m_context_table->setItem(i, 2, new QTableWidgetItem(QStringLiteral("FPR%1").arg(i)));
    m_context_table->setItem(i, 3, new QTableWidgetItem(get_f64_idx(context->fpr, i)));
    m_context_table->setItem(i, 4, new QTableWidgetItem(QStringLiteral("PSF%1").arg(i)));
    m_context_table->setItem(i, 5, new QTableWidgetItem(get_f64_idx(context->psf, i)));

    if (i < 8)
    {
      m_context_table->setItem(i, 6, new QTableWidgetItem(QStringLiteral("GQR%1").arg(i)));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex_idx(context->gqr, i)));
      continue;
    }
    switch (i)
    {
    case 8:
      ...
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->cr)));
      break;
    case 9:
      m_context_table->setItem(i, 6, new QTableWidgetItem(QLatin1Literal("LR")));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->lr)));
      break;
    case 10:
      m_context_table->setItem(i, 6, new QTableWidgetItem(QLatin1Literal("CTR")));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->ctr)));
      break;
    case 11:
      m_context_table->setItem(i, 6, new QTableWidgetItem(QLatin1Literal("XER")));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->xer)));
      break;
    case 12:
      m_context_table->setItem(i, 6, new QTableWidgetItem(QLatin1Literal("FPSCR")));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->fpscr)));
      break;
    case 13:
      m_context_table->setItem(i, 6, new QTableWidgetItem(QLatin1Literal("SRR0")));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->srr0)));
      break;
    case 14:
      m_context_table->setItem(i, 6, new QTableWidgetItem(QLatin1Literal("SRR1")));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->srr1)));
      break;
    case 15:
      m_context_table->setItem(i, 6, new QTableWidgetItem(QLatin1Literal("DUMMY")));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->dummy)));
      break;
    case 16:
      m_context_table->setItem(i, 6, new QTableWidgetItem(QLatin1Literal("STATE")));
      m_context_table->setItem(i, 7, new QTableWidgetItem(get_hex(context->state)));
      break;
    default:
      for (int j = 6; j <= 7; j++)
      {
...
void ThreadWidget::UpdateThreadCallstack(const Common::Debug::PartialContext* context)
{
  ...
  for (int i = 0; i < 16; i++)
  {
    ...
    m_callstack_table->setItem(i, 0, new QTableWidgetItem(get_hex(sp)));
    if (PowerPC::HostIsRAMAddress(sp + 4))
    {
      const u32 lr_save = PowerPC::HostRead_U32(sp + 4);
      m_callstack_table->setItem(i, 2, new QTableWidgetItem(get_hex(lr_save)));
    ...
    m_callstack_table->setItem(i, 1, new QTableWidgetItem(get_hex(sp)));
  }
}
