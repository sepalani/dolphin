...
using OSThreadQueue = Queue<OSThread>;
using OSThreadLink = Link<OSThread>;

using OSMutexQueue = Queue<OSMutex>;
using OSMutexLink = Link<OSMutex>;
...
struct OSContext
{
  ...
  static void Read(u32 addr, OSContext* ctx);
};
...
struct OSThread
{
  ...
  static void Read(u32 addr, OSThread* thread);
  static bool IsValid(const OSThread* thread);
};
...
struct OSMutex
{
  ...
  static void Read(u32 addr, OSMutex* mutex);
};
